...
// 값 입력/출력용 클래스 : 사용자와 맞닿는 클래스(User Interface)
public class EmployeeView {
	...
	public void displayMenu() {
		...
		do {
			try {
				System.out.println("\n----------------------------------\n");
				System.out.println("\n----------사원 관리 프로그램-----------\n");
			System.out.println("1. 전체 사원 조회"); // 사번, 이름, 부서명, 직급명, 전화번호 / 직급코드 오름차순(J1~)
			System.out.println("2. 사번이 일치하는 사원 조회"); // 사번, 이름, 부서명, 직급명, 전화번호 1행 조회
			System.out.println("3. 이름에 글자가 포함된 사원 조회"); // 사번, 이름, 부서명, 직급명, 전화번호 n행 조회 / 사번 오름 차순
			System.out.println("4. 급여 범위로 조회"); // 사번, 이름, 부서명, 직급명, 급여 조회 / 급여 내림차순 // 먼저 입력한 값 이상, 나중에 입력한 값 이하
			System.out.println("5. 사원 정보 추가"); // INSERT, SEQUENCE 사용
			System.out.println("6. 사번으로 사원 정보 수정"); // 이메일, 전화번호, 급여 수정
			System.out.println("7. 사번으로 사원 퇴사"); // ENT_YN -> 'Y'로, ENT_DATE -> 'sysdate' 수정 
	...
	private void selectAll() {
		// 사번, 이름, 부서명, 직급명, 전화번호 / 직급코드 오름차순(J1~)
		System.out.println("\n-----------전체 사원 조회------------\n");
		
		try {
		// DB에서 전체 사원 정보를 조회하는 service selectAll()을 호출하여 결과 반환 받기
	...
	private void selectOne() {
		...
		try {
			...
			System.out.println("[사번으로 사원 조회 중 예외 발생]");
			e.printStackTrace();
		}
	}
	...
	private void selectName() {
		...
		// Service 메서드에 단어를 전달해서 단어가 일치하는 사원 정보를 반환
		
		try {
			List<Employee> empList = service.selectName(input);
			
			if(empList.isEmpty()) { // 조회 결과가 없는 경우
	...
	private void selectSalary() {
		...
		try {
			...
			if (empList.isEmpty()) { // 조회 결과가 없을 경우(0행)
				System.out.println("[입력된 범위에 포함되는 사원이 없습니다.]");
				return;
			}
	...
	private void insertEmployee() {
		System.out.println("\n-----------사원 추가-----------\n");
	      ...
	      Employee emp = new Employee(empName, empNo, email, phone, deptCode, jobCode, salLevel, salary, bonus, managerId);
	      
	      // 사원 정보를 DB에 삽입하는 서비스 호출 후 결과 반환 받기
	      try {
		      int result = service.insertmEmployee(emp);
		      if(result > 0) {
	...
	private void retireEmployee() {
		...
		System.out.print("퇴사처리할 사원의 사번 입력 : ");
		int input = sc.nextInt();

...
public class StudentView {
	...
	private StudentService service = new StudentService(); // 기본 생성자를 이용해 생성
	...
	public void displayMenu(){
		...
		do {
			...
			System.out.println("6. 학생 총점 점수 합계 ,평균, 최고점, 최저점 조회");
			...
			case 3 : selsectIndex(); break;
			case 4 : selectName(); break;
			case 5 : updateStudet(); break;
	...
	private void selectAll() {
		...
		// **************************************************************
		// 매개변수, 반환값으로 참조변수가 사용될 경우 참조하고있는 주소가 복사 된다 -> 얕은 복사
		// **************************************************************
		Student[] studentList = service.selectAll(); // 얕은 복사가 일어나고 있다
		
		
		for(int i=0; i<studentList.length; i++) {
			if (studentList[i] == null) {
				break; // 참조하는 객체가 없으면 반복을 종료
			}
			// Student 객체가 가지고 있는 toString() 메서드를 호출해서 학생 정보를 문자열로 반환 받아 출력
			System.out.println(studentList[i].toString());
			
			// NullPointerException
			// 참조하고 있는 객체가 없는 참조변수를 이용해서 필드/메서드를 호출할 때 발생하는 에러(예외)
	...
	/**3. 학생 정보 조회(인덱스)	 */
	private void selsectIndex() {
		System.out.println("[학생 정보 조회(인덱스)]");
		System.out.print("조회할 학생의 인덱스 번호를 입력하세요 : ");
		int index = sc.nextInt(); // index 번호를 service로 보내서, service에서 해당 index의 학생 정보를 받아오면 됨
		
		// 인덱스에 해당하는 배열요소를 얻어와 저장 
		Student std = service.selectIndex(index);
		
		if(std != null) { // 반환 받은 index번째 요소가 null이 아닐 경우(학생 정보가 있을 경우) ->NullPointerException 방지 처리
	...
	private void selectName() {
		...
		if (resultArr != null) {
			...
			// -> 배열에서 꺼내는 순서는 0부터 1씩 증가하여 마지막 index까지
			// -> 순차 접근(반복 접근) : 모든 요소에 접근을 함
			for(Student s : resultArr) {
				// s : resultArr의 요소가 for문 반복마다 하나씩 저장되는 변수
				if (s == null) break;
				// if / else if /else / for / while 등 {}블록을 작성하는 구문에서 {}블록을 생략하면 다음 한 줄에만 해당 구문을 적용시킨다
				// (한 줄의 기준은 세미콜론 전 까지)
				System.out.println(s.toString());
			}
		}
		else { // 없을 경우
			System.out.println("해당 이름의 학생 정보가 존재하지 않습니다.");
		}
	...
	private void sumAvgMaxMin() {
		System.out.println("[학생 총점 점수 합계, 평균, 최고점, 최저점 조회]");
		int[] arr = service.sumAvgMaxMin();
		
		// 0번 인덱스 : 학생 총점 합계
		// 1번 인덱스 : 학생 총점 평균(정수로만 처리)
		// 2번 인덱스 : 학생 총점 최고점
		// 3번 인덱스 : 학생 총점 최저점
		
		System.out.println("학생 총점 합계 : " + arr[0]);
		System.out.println("학생 총점 평균 : " + arr[1]);
		System.out.println("학생 총점 최고점 : " + arr[2]);
		System.out.println("학생 총점 최저점 : " + arr[3]);

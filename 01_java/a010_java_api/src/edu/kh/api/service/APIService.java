...
public class APIService {
	...
	// 프로그래밍 언어가 이미 가지고 있는 클래스/기능/기술을 사용자가 쉽게 사용할 수 있도록 제공하는 것
	...
	/**학생　추가　서비스　（단축키 alt shit J）
	 * @param grade
	 * @param classRoom
	 * @param number
	 * @param name
	 * @return　추가 성공 시 true / 실패 시 false (중복되는 학생이 있을 시 실패)
	 */
	public boolean addStudent(int grade, int classRoom, int number, String name) {
		
		int index = 0; // 비어있는 (studentList[index]값이 null인) 인덱스 저장
		for (Student s : studentList) {
			...
			// 학생 배열에 입력받은 학생이 존재 한다면
			if(check1 && check2 && check3 && check4) return false;
			index++;
			
		}
		
		// 학생 배열에 학생이 가득 차 자리가 없는 경우
	...
	public boolean addStudent2(int grade, int classRoom, int number, String name) {
		...
		for(Student s : studentList) {
			...
			// 중복된 학생이 있는 경우
			//if (s.equals(temp)) return false;  // s가 참조하는 학생과, temp가 참조하는 학생이(필드 값이) 같을 경우(동등한 경우) : 중복
	...
	public Student[] selectName(String input) {
		...
		// String .substring(0,4) 0번부터 4칸
		// String.split([구분자]) : 문자열을 구분자를 기준으로 나누어 String[] 형태로 반환
		String[] names = input.split(",/|"); // ,  또는 / 또는 |를 기준으로 구분
	...
	public String printName() {
		...
		// String.join("구분자", Stromg[])
		// String[]의 요소를 하나의 문자열로 합침. 단, 요소 사이사이에 "구분자" 추가
		// ex) String[] arr = {"aaa", "bbb", "vvv"}
		//	   String.join("@", arr) -> aaa@bbb@vvv

...
CREATE TABLE MEMBER (
	...
	MEMBER_NAME VARCHAR(15), -- 한글 2 글지 ~ 5글자(3BYTE)
	MEMBER_SSN CHAR(14), --항상 14글자
	ENROLL_DATE DATE DEFAULT SYSDATE -- 가입일 
);

-- DEFAULT (기본값) : INSERT/UPDATE 시 해당 컬럼의 값을 작성하지 않거나 DEFAULT로 작성하면 DEFAULT 오른쪽에 작성된 값이 컬럼에 반영됨
-- 만든 테이블 확인
SELECT * FROM MEMBER;
...
INSERT INTO MEMBER2 VALUES ('고길동', 9001011234567, 01012341234); -- 9001011234567, 1012341234
INSERT INTO MEMBER2 VALUES ('김길동', 0001013234567, 01012341234); -- 1013234567, 1012341234
...
CREATE TABLE USER_USED_NN(
    USER_NO NUMBER NOT NULL , -- > 컬럼 레벨 제약조건 설정 (NOT NULL은 컬럼 레벨로만 작성 가능)
    USER_ID VARCHAR2(20) ,
...
--  USER_ID VARCHAR2(20) CONSTRAINT USER_ID_UK UNIQUE, -- 컬럼 레벨 제약조건 설정(이름 0)
    USER_ID VARCHAR2(20), 
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    ...
    EMAIL VARCHAR2(50),
    -- 테이블 레벨 제약조건 설정
    CONSTRAINT USER_ID_UK UNIQUE (USER_ID) -- 제약조건(컬럼명) (이름 미지정)
);
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJM.SYS_C0031040)에 위배됩니다
-- 
...
CREATE TABLE USER_USED_UK2(
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    CONSTRAINT USER_ID_NAME_UK UNIQUE (USER_ID, USER_NAME) -- 테이블 레벨 복합키 지정
);
...
--    USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨 (제약조건명 지정 O)
    USER_NO NUMBER,
    ...
    PHONE VARCHAR2(30),
    EMAIL VARCHAR2(50),
    CONSTRAINT USER_NO_PK PRIMARY KEY (USER_NO) -- 테이블 레벨 (제약조건명 지정 X)
);
...
--> 기본키 중복으로 오류
--> SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJM.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
--> SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJM"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- SQL Error [1] [23000]: ORA-00001: 무결성 제약 조건(KH_KJM.PK_USERNO_USERID)에 위배됩니다

SELECT * FROM USER_USED_PK2;

INSERT INTO USER_USED_PK2
VALUES(NULL, 'user01', 'pass01', '신사임당', '여', '010-9999-9999', 'sin123@kh.or.kr');
-- PRIMARY KEY는 NULL이 들어갈 수 없음 (둘 중에 하나라도 NULL이 들어가면 안 됨)
-- 복합키로 PK가 설정되어 있어도 결국은 PK ==> NULL 삽입 불가능
-- SQL Error [1400] [23000]: ORA-01400: NULL을 ("KH_KJM"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
--> NULL 사용 가능(여기서의 NULL은 외부 테이블에 참조할 값이 없다라는 뜻)
...
--> SQL Error [2291] [23000]: ORA-02291: 무결성 제약조건(KH_KJM.GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
WHERE GRADE_CODE = 20;
-- > GRADE_CODE = 20 행은 자식 테이블에서 참조하고 있지 않으므로 삭제가 제한되지 않음

-- 참조를 하고 있는 GRADE_CODE 10을 삭제
DELETE FROM USER_GRADE
WHERE GRADE_CODE = 10;
-- SQL Error [2292] [23000]: ORA-02292: 무결성 제약조건(KH_KJM.GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
WHERE GRADE_CODE = 10;
-- > 삭제 성공, USER_USED_FK2에서 10이 NULL값으로 변경
...
CREATE TABLE USER_USED_FK3(
  ...
  GRADE_CODE NUMBER CONSTRAINT GRADE_CODE_FX3 REFERENCES USER_GRADE3 ON DELETE CASCADE
...
CREATE TABLE USER_USED_CHECK(
  ...
  USER_PWD VARCHAR2(30) NOT NULL,
  USER_NAME VARCHAR2(30),
  GENDER VARCHAR2(10) CONSTRAINT GENDER_CHECK CHECK (GENDER IN ('남', '여')), -- 남, 여 둘 중에 하나만 들어오게
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50)
);
...
-- SQL Error [2290] [23000]: ORA-02290: 체크 제약조건(KH_KJM.GENDER_CHECK)이 위배되었습니다
...
CREATE TABLE USER_TEST(
	USER_NO NUMBER CONSTRAINT USER_NO_TEST PRIMARY KEY,
	USER_ID VARCHAR2(20) CONSTRAINT UK_USER_ID UNIQUE,
	USER_PWD VARCHAR2(20) CONSTRAINT NN_USER_PWD NOT NULL,
	PNO VARCHAR2(20) CONSTRAINT NN_PNO NOT NULL CONSTRAINT UK_PNO UNIQUE,
	GENDER VARCHAR2(3) CONSTRAINT CK_GENDER CHECK (GENDER IN ('남', '여')),
	PHONE VARCHAR2(20),
	ADDRESS VARCHAR2(100),
	STATUS CHAR(3) CONSTRAINT NN_STATUS NOT NULL CONSTRAINT CK_STATUS CHECK (STATUS IN ('Y', 'N'))
	);

SELECT * FROM USER_TEST;

INSERT INTO USER_TEST (USER_NO, USER_ID, USER_PWD, PNO, GENDER, PHONE, ADDRESS, STATUS)
VALUES (1, 'MEM01', 'QWER1234', '123456-7891011', '남', '010-1234-5678', '서울시 종로구', 'Y');
INSERT INTO USER_TEST (USER_NO, USER_ID, USER_PWD, PNO, GENDER, PHONE, ADDRESS, STATUS)
VALUES (2, 'MEM02', 'QWER5678', '789456-7891011', '여', '010-5432-5678', '서울시 영등포구', 'N');
INSERT INTO USER_TEST (USER_NO, USER_ID, USER_PWD, PNO, GENDER, PHONE, ADDRESS, STATUS)
VALUES (3, 'MEM03', 'QWER9123', '425841-7891011', '여', '010-9485-5678', '서울시 중구', 'Y');
INSERT INTO USER_TEST (USER_NO, USER_ID, USER_PWD, PNO, GENDER, PHONE, ADDRESS, STATUS)
VALUES (4, 'MEM04', 'QWER7456', '943215-7891011', '남', '010-7894-5678', '서울시 용산구', 'N');
...
AS (SELECT * FROM EMPLOYEE); -- 서브쿼리의 결과(RESULT SET)의 형태로 테이블 생성
...
-- 3) 테이블의 형태 (컬럼명, 데이터 타입)만 복사 (컬럼 값 복사 X)
CREATE TABLE DEPARTMENT_COPY AS
SELECT * FROM DEPARTMENT 
WHERE 1 = 0 ; -- WHERE절 조건이 FALSE가 나오는게 중요
...
FROM USER_CONSTRAINTS C1
JOIN USER_CONS_COLUMNS C2 USING(CONSTRAINT_NAME)
WHERE C1.TABLE_NAME = 'EMPLOYEE_COPY'; -- PK가 없다
...
FOREIGN KEY (DEPT_CODE) REFERENCES DEPARTMENT(DEPT_ID); -- 관계를 설정하면 JOIN이 가능
...
FOREIGN KEY (LOCATION_ID) REFERENCES LOCATION ; -- 테이블만 작성하면 자동으로 PK가 연결

...
-- 23명에서 DEPT_CODE가 NULL값인 2명은 제외된 21명의 결과가 나옴
...
-- NULL인 애도 포함시키고 싶다 -> OUTER JOIN
-- 기본값이 INNER JOIN이라 생략
...
-- 15행. MANAGER ID가 없는 사람들(NULL)은 제외 -> LEFT JOIN으로 변경 (23명)


-- 오라클 구문
SELECT EMP.EMP_ID, EMP.EMP_NAME , NVL(EMP.MANAGER_ID, '없음'), NVL(MGR.EMP_NAME , '없음')
FROM EMPLOYEE EMP, EMPLOYEE MGR
WHERE EMP.MANAGER_ID  = MGR.EMP_NAME (+);
...
-- DEPARTEMNT : DEPT_ID CHAR(2) D1~D9
-- 이름이 다른 컬럼 -> NATURAL JOIN이 성립되지 않음 -> CROSS JOIN 결과 조회
...
-- EMPLOYEE , DEPARTMENT , LOCATION 테이블 조인해보기
-- ANSI 표준
...
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE);
-- * JOIN 순서 중요! 
-- 앞선 두 테이블(EMPLOYEE, DEPARTMENT)의 조인 후 JOIN된 결과에 LOCATION 테이블을 또 JOIN
...
SELECT EMP_ID, EMP_NAME, JOB_NAME , DEPT_TITLE , NATIONAL_CODE , SALARY
FROM EMPLOYEE 
NATURAL JOIN JOB
JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
-- 오라클 전용
SELECT E.EMP_ID , E.EMP_NAME, J.JOB_NAME , D.DEPT_TITLE , L.NATIONAL_CODE , SALARY
...
-- 어차피 INNER JOIN 하면 사라지기 때문에 WHERE절 필요 없음
...
ORDER BY EMP_ID ;
-- SELECT 절에 EMP_ID가 없어도 ORDER BY 절에 사용 가능
...
-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름을 조회하시오. PDF랑 안 맞음 확인 필
-- SELF JOIN 사용

SELECT O.EMP_NAME , O.DEPT_CODE , C.EMP_NAME  
FROM EMPLOYEE O
JOIN EMPLOYEE C ON (O.DEPT_CODE = C.DEPT_CODE)
WHERE O.EMP_ID != C.EMP_ID -- 사번으로 구분하는게 좋음(동명이인이 있을 수도 있으니까) 
ORDER BY 1;

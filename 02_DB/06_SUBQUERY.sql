/*
    * SUBQUERY (서브쿼리)
    - 하나의 SQL문 안에 포함된 또다른 SQL문(오직 SELECT문만)
...
-- ROWNUM 행번호를 출력하는 가상의 컬럼 (주의사항!! : ORDER BY절 해석 전에 먼저 행번호 부여->ORDER BY 하면 번호가 섞임)
...
-- SELECT 절에 작성된 그룹 함수 이외의 컬럼을 모두 GROUP BY 절에 작성해야, GROUP이 구성된다
...
-- 2) UNION이 아닌 선택함수(DECODE, CASE) 를 이용하는 방법 (이쪽이 더 빠르고 효율적)
SELECT EMP_ID , EMP_NAME , DEPT_TITLE , JOB_NAME, 
	CASE 
...
FROM EMPLOYEE 
NATURAL JOIN JOB
WHERE JOB_NAME = '대리' AND SALARY > (SELECT MIN(SALARY) FROM EMPLOYEE NATURAL JOIN JOB WHERE JOB_NAME = '과장');
...
FROM EMPLOYEE 
NATURAL JOIN JOB
WHERE JOB_NAME = '대리' AND SALARY > ANY (SELECT SALARY FROM EMPLOYEE NATURAL JOIN JOB WHERE JOB_NAME = '과장');
...
SELECT DEPT_ID 
FROM DEPARTMENT 
WHERE LOCATION_ID = (SELECT LOCAL_CODE FROM LOCATION WHERE NATIONAL_CODE = 'KO'); -- 5행(다중행)
...
-- 1) 퇴사한 여직원 조회 퇴사여부 ENT_YN = Y, 퇴사일 ENT_DATE
...
-- EXIST : 서브쿼리에서 조회된 결과와 일치하는 행이 메인 쿼리에 하나라도 있으면 조회 결과에 포함
--		   서브쿼리의 조회 결과가 1행 이상이면, 메인 쿼리 행을 결과에 포함			
...
WHERE HIRE_DATE = 
				...
				--WHERE (ENT_YN = 'N' AND SUB.DEPT_CODE = MAIN.DEPT_CODE) -- NULL 이 아닌 경우
				--OR (ENT_YN = 'N' AND SUB.DEPT_CODE IS NULL AND MAIN.DEPT_CODE IS NULL) -- NULL인 경우
				WHERE (ENT_YN = 'N' AND NVL(SUB.DEPT_CODE, '1') = NVL(MAIN.DEPT_CODE, '1')) --도 가능
				)
ORDER BY HIRE_DATE ;
-- 상관쿼리 사용방법
-- 어렵지만 신뢰도 높음
...
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING (JOB_CODE)
WHERE (DEPT_CODE , HIRE_dATE) IN (SELECT DEPT_CODE , MIN(HIRE_DATE) FROM EMPLOYEE WHERE ENT_YN = 'N' GROUP BY DEPT_CODE)
ORDER BY HIRE_DATE ;
-- 쉬운 방법
-- 신뢰도가 떨어짐 (A부서의 최소값 데이터가 B부서의 비교 대상이랑 우연히 일치할 가능성도 있기 때문) 
-- => 다중행, 다중열로 변경 가능(BUT 그렇게 하면 NULL값이 포함되지 않음. NULL값은 IS, IS NULL을 사용하지 않을 시 연산 불가능)
----------------------------------------------------------------------------------
...
FROM EMPLOYEE MAIN;
-- 스칼라 서브쿼리 + 상관쿼리
-- 게시글에 댓글 수, 신고 수, 좋아요 수 표기할 때 주로 씀
...
SELECT EMP_ID , EMP_NAME , NVL(MANAGER_ID, '-') , 
	NVL ((SELECT EMP_NAME FROM EMPLOYEE SUB WHERE MAIN.MANAGER_ID = SUB.EMP_ID), '-') 관리자명
FROM EMPLOYEE MAIN;

SELECT MAIN.EMP_ID , MAIN.EMP_NAME, NVL(MAIN.MANAGER_ID, '없음') 관리자아이디, NVL(SUB.EMP_NAME, '없음') 관리자명
FROM EMPLOYEE MAIN
LEFT JOIN EMPLOYEE SUB ON (MAIN.MANAGER_ID = SUB.EMP_ID)
...
-- * 주의사항 : 
-- 조건절에 ROWNUM 사용 시, 반드시 조건 범위 내에 1 (1번 행) 이 반드시 포함되어야 한다.




-- 급여 평균이 3위 안에 드는 부서의 부서코드와 부서명, 평균급여를 조회
-- 1) 1,2,3 번을 토대로 급여 상위 5명 조회 시도
SELECT EMP_NAME , SALARY , ROWNUM
FROM EMPLOYEE 
WHERE ROWNUM <=5
ORDER BY SALARY DESC;
-- EMPLOYEE 테이블 위에서 부터 5명을 가지고 급여 내림차순 조회
-- 문제원인 : SELECT 해석 순서가 가장 마지막으로 해석되기 때문에 급여 상위 5명이라는  의도와 맞지 않는 형태로 조회
-- 해결방법 : 인라인뷰(from절에 작성되는  서브쿼리)를 이용하여 해결
-- 해결 1 ) 급여 내림차순 조회
SELECT EMP_NAME , SALARY 
FROM EMPLOYEE 
ORDER BY SALARY DESC;
-- > 해결 1의 조회 결과 (RESULT SET)를 가상의 테이블인 VIEW로 이용 할 예정 ( 행 순서, 컬럼 그대로 이용)

-- 해결 2) 해결 1의 조회 결과를 FROM 절에 작성하여 상위 5행만 ㅈ회

SELECT EMP_NAME, SALARY
FROM (SELECT EMP_NAME, SALARY FROM EMPLOYEE ORDER BY SALARY DESC)  --> 메인커리에 포함된 VIEW 생성 구분(인라인뷰)
WHERE ROWNUM <= 5;

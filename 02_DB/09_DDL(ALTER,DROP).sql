...
ALTER TABLE DEPT_COPY ADD CONSTRAINT PI_DEPT_COPY PRIMARY KEY (DEPT_ID);
...
ALTER TABLE DEPT_COPY ADD CONSTRAINT CK_LOCATION_ID CHECK (LOCATION_ID IN ('L1', 'L2', 'L3', 'L4', 'L5'));
...
-- ** NULL 설정 시에만 MODIFY를 사용 **
---------------------------

-- DEPT_COPY에 추가한 제약조건 중 PK 빼고 모두 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT UK_DEPT_TITLE;
ALTER TABLE DEPT_COPY DROP CONSTRAINT CK_LOCATION_ID;
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C0031216;
-- NOT NULL 제약조건 : ADD는 안 되는데 DROP은 가능

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY MODIFY DEPT_TITLE CONSTRAINT SYS_C0031216 NULL;
-- 이런식으로도 사용 가능
...
-- 한글은 1글자에 3바이트, BUT 부서명은 이미 더 큰 사이즈로 존재
...
SELECT * FROM DEPT_COPY; -- '한국'상태에서 안 바뀜. 기본값만 바뀌었기 때문
...
-- DDL : 데이터 정의 언어. 객체를 CREAT, ALTER, DROP 
-- DML : 데이터 조작 언어. 테이블에 들어가는 값을 INSERT, UPDATE, DELETE
-- TCL : 트랜잭션 제어 언어. DML 수행 내용을 COMMIT, ROLLBACK, SAVEPOINT

-- DML들이 트랜잭션에 쌓여있는 상황에서 DDL이 실행되면 이 DDL이 트랜잭션을 밀고 DB에 들어가 자동 COMMIT을 시켜버림

...
const SignupContainer = () => {
    ...
    const idCheck = (inputId) => {
        ...
        .then(result => {
            ...
            if(Number(result) === 0) { // 중복 아님 (사용 가능)
    ...
    const signup = () => {
        ...
        // 1. 비밀번호가 일치하지 않으면 '비밀번호가 일치하지 않습니다' alert로 출력 후 return (pw === pwCheck)
        if(pw === pwCheck) {
    ...
    return(
        <div className='signup-container'>
            ...
            <label>
                PW : <input type="password" onChange={ e => {setPw(e.target.value)}} value={pw}></input>
            </label>
            <label>
                PW CHECK : <input type="password" onChange={ e => {setPwCheck(e.target.value)}} value={pwCheck}></input>
            </label>
            <label>

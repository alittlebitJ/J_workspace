...
document.getElementById("btn1").addEventListener("click", () => {
    ...
    // - 요소.childNodes : 요소의 자식 노드를 모두 반환(NodeList형식으로 반환)
    ...
    // 탐색 구문은 연달아서 작성 가능
    console.log(list[11].firstChild.nedtSibling.nedtSibling.nedtSibling.nedtSibling.nedtSibling.nedtSibling)
    /* 탐색 범위를 넘어가면 null 반환 */
});
...
document.getElementById("btn2").addEventListener("click", () => {
    ...
    // #test의 첫번째 자식 요소의 다음 형제 요소에게 클릭 시 alert()가 출력되는 이벤트 핸들러 추가
...
btn4.addEventListener("click", () => {
    // 1) div 요소 만들기 (아직 화면에 추가는 되지 않음)
    // 요소만 만드록 위치를 지정하지 않은 상태
    ...
    // -> child를 화면에 추가를 했다고 해서 해당 코드에서 추적 못 하는 것은 아님
    //    child 변수를 이용해서 계속 접근 가능
    child.innerText = count2++;

    //** 3번이랑 2번의 순서를 바꾸어도 상관 없음 **/
});
...
document.getElementById("calc").addEventListener("click", () => {
    ...
    const list = document.getElementsByClassName("in"); // 클릭하는 시점에 input태그를 읽어오는게 좋음
    let sum = 0;
...
// <div class="row"><input type = "number class="in"></input></div> 추가하기
...
document.getElementById("add").addEventListener("click", () => {
    ...
    // 부모요소.prepend(자식요소) : 부모 요소의 첫 번째 자식으로 자식 요소를 추가
    ...
    // span에 $times; 내용 추가(innerHTML)로 사용해야
    span.innerHTML = "&times";
    // 만들어진 span 요소에 이벤트리스너를 추가
    span.addEventListener("click", e => {
        // 클릭된 요소의 부모 요소 삭제 (row 삭제)

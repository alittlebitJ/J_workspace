...
function clickCount(a) {
    ...
    // a = <div id="btn1" onclick="clickCount(this)">0</div> 전체
    a.innerText = Number(a.innerText) + 1;
    if(Number(a.innerText) >20){
        a.innerText = 0;
        return;    
    }
}
...
// - 화살표 함수 : (e) => {}
//                   e => {}
// 3. 매개 변수가 2개 이상 또는 0개이면 () 생략 불가능    (e, temp)
...
document.querySelector("#button4").addEventListener("click", () => {
    ...
    printConsole(num => {return num*5}); // 매개변수 1개 -> ()생략
    printConsole(num =>  num*2); // 함수 정의 부분에 return만 작성 -> {}, return 생략 가능
    ...
    // -> JS 객체의 {}를 함수 저으이 부분의 {}로 인식해서 구문 오류 발생
...
// 클ㅊ된 요소의 배경색을 검정색으로 변경(익명)
button6.addEventListener("click", function(e){
    e.target.style.backgroundColor = "black";
});

// 클릭된 요소의 글자색을 하얀색으로 변경(화살표)
...
button6.addEventListener("click", () => {
    console.log(this);
    // 화살표 함수에 this를 작성하면 이벤트가 발생한 요소가 아닌 창 자체를 나타내는 window라는 객체가 반환된다

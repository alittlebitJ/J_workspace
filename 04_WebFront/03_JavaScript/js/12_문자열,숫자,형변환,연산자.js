...
document.querySelector("#btn2").addEventListener("click", function(e){
    // Infinity 리터럴(값) 확인
    console.log(5 / 0);
    // Java : ArithmerticException(산술적 예외)
    // JavavScript : Infinity
    ...
    // round는 무조건 첫째자리에서 반올림

    // 1.25를 소숫점 둘째자리에 반올림
    // -> 1.3이 나오도록
    console.log(Math.round(1.25*10)/10);

    // 부동 소수점(부정확, 저용량) -> 고정 소수점(정확, 고용량) 변경
    // 숫자.toFixed(보여질 소수 자릿수)  : 
    // 주어진 숫자를 매개변수에 작성된 숫자 만큼 소수점 자리수로 표현될 수 있도록
    // 바로 다음 소수자리에서 반올리해서 문자열로 반환

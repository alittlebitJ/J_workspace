...
//		작성하기 힘드니까 JSP로 위임
//		Request		: 요청
//		Dispatcher	: 발송자, 필요한 정보를 제공하는 자, 역할을 넘간다
		
//		RequestDispatcher : 요청정보, 응답하는 역할을 넘기는 객체
...
//		forward를 하면 주소창 주소가 요청 주소로 바뀌고 그 요청에 맞는 화면으로 변함
//		(요청에 맞는 응답을 했기 때문에)
		
		
//		Servlet이 가지고 있던 HttpServletreq, HttpServletresp 객체를 JSP에 전달하여 요청을 실제로 위임
		
//		-----------------------------

//		기존 req에 없던 값(message)을 속성(Attribute)으로 추가
//		Map 형식으로 작성
...
//		***** [redirect] *****
//		다시 다른 요청을 한다(재요청) : 
//		현재 요청("/login")에 대한 응답 화면이 따로 존재하지 않아 응답 화면이 있는 다른 요청 주소("/")로 다시 요청

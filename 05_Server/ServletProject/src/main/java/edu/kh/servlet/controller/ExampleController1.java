...
public class ExampleController1 extends HttpServlet {
	...
	// doGet() 메서드 : GET 방식 요청을 처리하는 메서드 (HttpServlet의 메서드를 오버라이딩)
	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		// HttpServletRequest : 클라이언트 요청 시 생성되는 객체. 클라이언트 데이터 + 요청 관련 데이터
		// HttpServletResponse : 클라이언트 요청 시 서버에서 생성하는 객체. 서버가 클라이언트에게 응답하기 위한 방법을 제공하는 객체
		...
		// 요청 시 전달된 데이터 중 name 속성 값이 일치하는 데이터의 value를 얻어 와 String 형태로 반환

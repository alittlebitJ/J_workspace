...
const preview = document.getElementsByClassName("preview"); // img 태그 5개
const inputImage = document.getElementsByClassName("inputImage"); // input : file 5개
const deleteImage = document.getElementsByClassName("delete-image"); // x 버튼 5개


// 게시글 수정 시 삭제된 이미지의 순서를 기록할 Set 객체 생성
const deleteSet = new Set(); // 순서 없음, 중복 없음 (x버튼 다회 클릭시 저장 중복 제거 용도로 set 사용)
...
for(let i=0; i<inputImage.length; i++) {
    ...
    deleteImage[i].addEventListener("click", () => {
        if(preview[i].getAttribute("src") != ""){ // 미리보기 이미지가 있을 경우
            // 미리보기 삭제
            preview[i].removeAttribute("src");

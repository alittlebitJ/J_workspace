// 댓글 목록 조회 (삽입, 수정, 삭제가 일어날 때 마다)
function selectCommentList(){
    /* 요청주소만 적으면 Get 방식 */
    // REST (REpresentaitional State Transfer) API
    // - 자원을 이름(주소)으로 구분하여 자원의 상태를 주고 받는 것
    // - 주소를 명시하고 Http Method (GET, POST, PUT, DELETE)를 이용해 지정된 자원에 대한 CRUD를 진행
    // ** CRUD :  Create(생성,삽입/POST), Read(조회/GET), Update(수정/PUT, PETCH), Delete (삭제/DELETE)
    // ** 기본적으로 form 태그는 GET/POST만 지원

                                // boardDetail.jsp에서 변수로 생성한 board.boardNo
    fetch("/comment?boardNo=" + boardNo) // GET방식은 주소에 파라미터를 담아 전달
    .then(response => response.json()) // 응답 객체를 필요한 행태로 파싱 (java의 list가 여기서 객체 배열 형태로 바뀜)
...
addComment.addEventListener("click", e => { // 댓글 등록 버튼이 클릭이 되었을 때
    ...
    const data = {"commentContent" : commentContent.value,
                  ...
                  "boardNo" : boardNo}; // JS 객체를 생성

    fetch("/comment", {
        method : "POST",
        headers : {"Content-Type" : "application/json"},
        body : JSON.stringify(data) // JS객체를 JSON으로 파싱
    })
    .then(resp => resp.text())
    .then(result => {
        if(result > 0){ // 등록 성공
...
function deleteComment(commentNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        fetch("/comment", {
            method : "DELETE",
            headers : {"Content-Type" : "application/json"},
            body : commentNo /* 객체 형식이 아니라 값 하나만 담아서 전달(값 하나만 보내고 싶을 때엔 JSON 필요x) */
        })
...
function showUpdateComment(commentNo, btn){
    ...
    // 1. 댓글 수정이 클릭된 행을 선택
    const commentRow = btn.parentElement.parentElement; // 수정 버튼의 부모의 부모(li)

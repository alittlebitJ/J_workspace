...
if(updateInfo != null) {
    ...
    memberNickname.addEventListener("input", () => {
        ...
        if(regEx.test(memberNickname.value)) { // 유효하다
    ...
    memberTel.addEventListener("input", () => {
        ...
        if(regEx.test(memberTel.value)) { // 유효하다
...
if(changePwFrm != null) { // 현제 페이지가 비밀번호 변경 페이지인 경우
...
if (secessionFrm != null) { // 탈퇴 페이지인 경우
    console.log("if문시작")
    const memberPw = document.getElementById("memberPw");// 중복될까봐 지역변수로 작성
    const agree = document.getElementById("agree");

    secessionFrm.addEventListener("submit", e => {
        console.log("이벤트엔 들어감")
        if(memberPw.value.trim() == "") { // 비밀번호 미작성
            console.log("비밀번호 미작성시")
            alert("비밀번호를 작성해주세요");
            e.preventDefault();
            memberPw.focus();
            return;
        ...
        if(!agree.checked) { //동의 체크가 되지 않은 겨우
            console.log("동의 체크가 되지 않은")

...
// 스프링이 일정 시간마다 해당 객체를 이용해서 코드를 수행하게
// 스프링이 해당 클래스를 객체로 만들어 관리를 해야 -> Bean등록
...
public class ImageDeleteScheduling {
	...
	// fixedDelay : 일(5초) -> 10초 대기 -> 일(5초) -> 10초 대기
	// fixedRate  : 일(5초) 
	//			    대기(10초)
	// cron		  : cron= "초 분 시 일 월 요일 [년도]"
...
//		System.out.println("스케쥴러가 일정 시간마다 자동으로 출력");
		...
		// 서버에 저장된 파일 목록을 조회해, DB에 저장된 파일 목록과 비교하여 매칭되지 않는 서버 파일 제거
		
		// 1) 서버에 저장된 파일 목록 조회
		// application 객체를 이용해서 /resources/imgaes/board의 실제 서버 경로를 얻어옴
		String filePath = servletContext.getRealPath("/resources/images/board");
		// C:\J_workspace\06_Framework\boardProject\src\main\webapp\resources\images\board
		
		// 2) filePath에 저장된 모든 파일 목록 읽어오기
		File path = new File(filePath); // 지정된 경로의 정보를 읽어오는 파일 객체를 만들고
...
//		4) 서버에 파일 목록이 있을 경우에 비교 시작(DB에는 없을수도 있음)
...
//				System.out.println(server.getName());
				...
				// List.indexOf(객체) : 객체가 Lsit에 있으면 해당 인덱스 반환, 없으면 -1반환
				if(dbImageList.indexOf(server.getName()) == -1) { // db파일 목록에 서버 파일 이름이 없을 때

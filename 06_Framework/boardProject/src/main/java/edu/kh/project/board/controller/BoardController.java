...
public class BoardController {
	 ...
	 * 
	 * ** 컨트롤러를 따로 생성 예정
	 * 게시글 삽입 : /board2/insert?code=1 (code == BOARD_CODE / 게시판 종류)
	 * 게시글 수정 : /board2/update?code=1$no=1500 (no == BOARD_NO / 게시글 번호)
	 * 게시글 삭제 : /board2/delete?code=1$no=1500
	 * 
	 * */
	
	
	
	// @PathVariable : 경로변수 어노테이션
	// URL 경로에 있는 값을 매개 변수로 이용할 수 있게 하는 어노테이션
	
	
	// @PathVariable을 사용하는 경우 
	// 자원(resource) 구분/식별이 필요한 경우
	// ex) github.com/testUser
	// ex) /board/1 → 1번(공지사항) 게시판
	// ex) /board/2 → 2번(자유) 게시판
	
	
	// Query String을 사용하는 경우(/board/1?key=value)
	// 식별을 제외한 나머지 상황
	// 정렬, 필터링

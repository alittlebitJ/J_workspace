...
//	@Before("execution(* edu.kh.project..*Impl*.*(..))") // 타겟 수행 전
	...
	@Order(1) // 순서, 하나의 타겟에 대한 여러 어드바이스 수행 시 순서 지정
	@Before("CommonPointcut.serviceImplPointcut()")
	// 지정된 클래스, 메서드에 작성된 @Pointcut() 어노테이션의 내용을 타겟으로 삼는다
	public void beforeLog (JoinPoint jp) { // Advice (수행할 코드)
		// 매개변수 JoinPoint : AOP의 부가 기능이 적용된 대상의 객체, 메서드, 파라미터 정보를 얻을 수 있게 해주는 객체
		// 대상 객체의 간단한 클래스명(패키지명 제외)
		String className = jp.getTarget().getClass().getSimpleName();
		// 메서드 선언부(시그니처)에서 메서드 명만 얻어옴

...
public class MyPageServiceImpl implements MyPageService {
	...
	// 비밀번호 변경 서비스
	@Transactional(rollbackFor = Exception.class) // transaction만 적을 시 Unchecked Exception 발생 시 rollback
	@Override
	public int changePw(String currentPw, String newPw, int memberNo) {
		
		// 1. 현재 비밀번호, DB에 저장된 비밀번호 비교
			// 1) 회원 번호가 일치하는 MEMber테이블 행의 MEMBER_PW 조회
		String encPw = dao.selectEncPw(memberNo);
			// 2) bcrypt.matches(평문, 암호문) -> 같으면 true(이 때 수정)
